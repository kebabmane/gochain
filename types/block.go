...
func (this *Block) Validation(parentBlock *Block) error {
	if parentBlock.Height+1 != this.Height {
		fmt.Println("height mismatch")
		return nil
	}
	//Hash() returns the value and an error TODO catch the error as opposed to just ignoring it
	tempParentHash, _ := parentBlock.Hash()
	if tempParentHash != this.ParentHash {
